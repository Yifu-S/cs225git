                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    else // singleQueue.front()->freq.getFrequency() <= mergeQueue.front()->freq.getFrequency()
    {
        smallest = singleQueue.front();
        singleQueue.pop();
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() != 1)
    {
        ...
        internal = new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
        internal->left = left;
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        else /* bfile.getNextBit() == 1 */  current = current->right;
