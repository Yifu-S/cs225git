...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* KDTreeConstructorHelper(vector<Point<Dim>>& newVector, int left, int right, int dimension);
    ...
    double pointDist(Point<Dim> p1, Point<Dim> p2) const;
    Point<Dim> findNearestNeighborHelper(KDTreeNode* curNode, const Point<Dim>& query, int dimension, double& radius, KDTreeNode*& currentBest) const;
    /**
    ...
    // Point<Dim> findSmallestHyperrectangle(KDTreeNode*& curNode, const Point<Dim>& query, int dimension) const;
  ...
  private:
    ...
    void clear();
    void KDTreeDestructorHelper(KDTreeNode* curNode);
    void copy(const KDTree<Dim>& other);
    void KDTreeCopyHelper(KDTreeNode*& curNode, KDTreeNode* otherNode);
